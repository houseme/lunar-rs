//! 皇纪（Koki / Japanese imperial year）。
...
impl KokiYear {
    pub fn from_year(year: i32) -> Result<Self, LunarError> {
        if year < YEAR_OFFSET + 1 {
            return Err(LunarError::Parse(format!("illegal koki year: {year}")));
        }
        Ok(Self { year })
    }

    pub fn from_solar(solar: Solar) -> Result<Self, LunarError> {
        let date = Koki::from_solar(solar)?;
    ...
    pub fn first_month(&self) -> KokiMonth {
        KokiMonth::from_ym(self.year, 1).unwrap()
    }

    pub fn last_month(&self) -> KokiMonth {
        KokiMonth::from_ym(self.year, 12).unwrap()
    }

    pub fn months(&self) -> Vec<KokiMonth> {
        (1..=12).map(|month| KokiMonth::from_ym(self.year, month).unwrap()).collect()
    }

    pub fn first_day(&self) -> Koki {
        self.first_month().first_day()
    }

    pub fn last_day(&self) -> Koki {
    ...
    pub fn contains_koki(&self, koki: Koki) -> bool {
        koki.year() == self.year
    }
...
impl CalendarSpan for KokiYear {
    fn first_solar_day(&self) -> Solar {
        KokiYear::first_solar_day(self)
    }

    fn last_solar_day(&self) -> Solar {
        KokiYear::last_solar_day(self)
    }
}

impl fmt::Display for KokiYear {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "皇纪{}年", self.year)
    }
}

#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash)]
pub struct KokiMonth {
    year: i32,
    month: i32,
}
...
impl KokiMonth {
    ...
    pub fn koki_year(&self) -> KokiYear {
        KokiYear::from_year(self.year).unwrap()
    }
    ...
    pub fn first_day(&self) -> Koki {
        Koki::from_ymd(self.year, self.month, 1).unwrap()
    }

    pub fn last_day(&self) -> Koki {
        Koki::from_ymd(self.year, self.month, self.day_count()).unwrap()
    }
    ...
    pub fn days(&self) -> Vec<Koki> {
        (1..=self.day_count()).map(|day| Koki::from_ymd(self.year, self.month, day).unwrap()).collect()
    }
    ...
    pub fn contains_koki(&self, koki: Koki) -> bool {
        koki.year() == self.year && koki.month() == self.month
    }
...
impl CalendarSpan for KokiMonth {
    fn first_solar_day(&self) -> Solar {
        KokiMonth::first_solar_day(self)
    }

    fn last_solar_day(&self) -> Solar {
        KokiMonth::last_solar_day(self)
    }
}

impl fmt::Display for KokiMonth {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{}{}月", self.koki_year(), self.month)
    }
}
...
impl Koki {
    ...
    pub fn from_solar(solar: Solar) -> Result<Self, LunarError> {
        if solar.year() < 1 {
            return Err(LunarError::Parse(format!("unsupported koki date: {}", solar.to_ymd())));
        }
    ...
    pub fn koki_month(&self) -> KokiMonth {
        KokiMonth::from_ym(self.year, self.month).unwrap()
    }
    ...
    pub fn to_string_in_lang(&self, language: crate::i18n::Language) -> String {
        crate::i18n::locale(language).render_koki_string(self)
    }
    ...
    pub fn to_full_string_in_lang(&self, language: crate::i18n::Language) -> String {
        crate::i18n::locale(language).render_koki_full(self)
    }
}

impl CalendarDay for Koki {
    fn solar(&self) -> Solar {
        Koki::solar(self)
    }
}

impl fmt::Display for Koki {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "皇纪{}年{}月{}日", self.year, self.month, self.day)
    }
}
