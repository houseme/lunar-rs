...
impl AucYear {
    pub fn from_year(year: i32) -> Result<Self, LunarError> {
        if year < YEAR_OFFSET + 1 {
            return Err(LunarError::Parse(format!("illegal auc year: {year}")));
        }
        Ok(Self { year })
    }
    ...
    pub fn first_month(&self) -> AucMonth {
        AucMonth::from_ym(self.year, 1).unwrap()
    }

    pub fn last_month(&self) -> AucMonth {
        AucMonth::from_ym(self.year, 12).unwrap()
    }

    pub fn months(&self) -> Vec<AucMonth> {
        (1..=12).map(|month| AucMonth::from_ym(self.year, month).unwrap()).collect()
    }

    pub fn first_day(&self) -> Auc {
        self.first_month().first_day()
    }

    pub fn last_day(&self) -> Auc {
    ...
    pub fn contains_auc(&self, auc: Auc) -> bool {
        auc.year() == self.year
    }
...
impl CalendarSpan for AucYear {
    fn first_solar_day(&self) -> Solar {
        AucYear::first_solar_day(self)
    }

    fn last_solar_day(&self) -> Solar {
        AucYear::last_solar_day(self)
    }
}

impl fmt::Display for AucYear {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "建城纪年{}年", self.year)
    }
}
...
impl AucMonth {
    ...
    pub fn auc_year(&self) -> AucYear {
        AucYear::from_year(self.year).unwrap()
    }
    ...
    pub fn first_day(&self) -> Auc {
        Auc::from_ymd(self.year, self.month, 1).unwrap()
    }

    pub fn last_day(&self) -> Auc {
        Auc::from_ymd(self.year, self.month, self.day_count()).unwrap()
    }
    ...
    pub fn days(&self) -> Vec<Auc> {
        (1..=self.day_count()).map(|day| Auc::from_ymd(self.year, self.month, day).unwrap()).collect()
    }
    ...
    pub fn contains_auc(&self, auc: Auc) -> bool {
        auc.year() == self.year && auc.month() == self.month
    }
...
impl CalendarSpan for AucMonth {
    fn first_solar_day(&self) -> Solar {
        AucMonth::first_solar_day(self)
    }

    fn last_solar_day(&self) -> Solar {
        AucMonth::last_solar_day(self)
    }
}

impl fmt::Display for AucMonth {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{}{}月", self.auc_year(), self.month)
    }
}
...
impl Auc {
    ...
    pub fn from_solar(solar: Solar) -> Result<Self, LunarError> {
        if solar.year() < 1 {
            return Err(LunarError::Parse(format!("unsupported auc calendar date: {}", solar.to_ymd())));
        }
    ...
    pub fn auc_month(&self) -> AucMonth {
        AucMonth::from_ym(self.year, self.month).unwrap()
    }
    ...
    pub fn to_string_in_lang(&self, language: crate::i18n::Language) -> String {
        crate::i18n::locale(language).render_auc_string(self)
    }
    ...
    pub fn to_full_string_in_lang(&self, language: crate::i18n::Language) -> String {
        crate::i18n::locale(language).render_auc_full(self)
    }
}

impl CalendarDay for Auc {
    fn solar(&self) -> Solar {
        Auc::solar(self)
    }
}

impl fmt::Display for Auc {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "建城纪年{}年{}月{}日", self.year, self.month, self.day)
    }
}
