...
impl FotoFestival {
    ...
    pub const fn every_month(&self) -> bool { self.every_month }
...
/// 佛历。借用底层 [`Lunar`]。
pub struct Foto<'a> { lunar: &'a Lunar }
...
impl<'a> Foto<'a> {
    ...
    pub fn is_month_zhai(&self) -> bool { let m = self.month(); m == 1 || m == 5 || m == 9 }
    pub fn is_day_yang_gong(&self) -> bool { self.festivals().iter().any(|f| f.name() == "杨公忌") }
    pub fn is_day_zhai_shuo_wang(&self) -> bool { let d = self.day(); d == 1 || d == 15 }
    pub fn is_day_zhai_six(&self) -> bool {
        let d = self.day();
        if d == 8 || d == 14 || d == 15 || d == 23 || d == 29 || d == 30 { return true; }
        if d == 28 {
    ...
    pub fn xiu(&self) -> &'static str { foto_util::get_xiu(self.month(), self.day()) }
