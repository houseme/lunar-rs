...
/// 檀纪年份 = 公历年份 + 2333。
pub const YEAR_OFFSET: i32 = 2333;

#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash)]
pub struct DangiYear {
    year: i32,
}
...
impl DangiYear {
    pub fn from_year(year: i32) -> Result<Self, LunarError> {
        if year < YEAR_OFFSET + 1 {
            return Err(LunarError::Parse(format!("illegal dangi year: {year}")));
        }
        Ok(Self { year })
    }
    ...
    pub fn first_month(&self) -> DangiMonth {
        DangiMonth::from_ym(self.year, 1).unwrap()
    }

    pub fn last_month(&self) -> DangiMonth {
        DangiMonth::from_ym(self.year, 12).unwrap()
    }

    pub fn months(&self) -> Vec<DangiMonth> {
        (1..=12).map(|month| DangiMonth::from_ym(self.year, month).unwrap()).collect()
    }

    pub fn first_day(&self) -> Dangi {
        self.first_month().first_day()
    }

    pub fn last_day(&self) -> Dangi {
    ...
    pub fn contains_dangi(&self, dangi: Dangi) -> bool {
        dangi.year() == self.year
    }
...
impl CalendarSpan for DangiYear {
    fn first_solar_day(&self) -> Solar {
        DangiYear::first_solar_day(self)
    }

    fn last_solar_day(&self) -> Solar {
        DangiYear::last_solar_day(self)
    }
}

impl fmt::Display for DangiYear {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "檀纪{}年", self.year)
    }
}

#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash)]
pub struct DangiMonth {
    year: i32,
    month: i32,
}
...
impl DangiMonth {
    ...
    pub fn dangi_year(&self) -> DangiYear {
        DangiYear::from_year(self.year).unwrap()
    }
    ...
    pub fn first_day(&self) -> Dangi {
        Dangi::from_ymd(self.year, self.month, 1).unwrap()
    }

    pub fn last_day(&self) -> Dangi {
        Dangi::from_ymd(self.year, self.month, self.day_count()).unwrap()
    }
    ...
    pub fn days(&self) -> Vec<Dangi> {
        (1..=self.day_count()).map(|day| Dangi::from_ymd(self.year, self.month, day).unwrap()).collect()
    }
    ...
    pub fn contains_dangi(&self, dangi: Dangi) -> bool {
        dangi.year() == self.year && dangi.month() == self.month
    }
...
impl CalendarSpan for DangiMonth {
    fn first_solar_day(&self) -> Solar {
        DangiMonth::first_solar_day(self)
    }

    fn last_solar_day(&self) -> Solar {
        DangiMonth::last_solar_day(self)
    }
}

impl fmt::Display for DangiMonth {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{}{}月", self.dangi_year(), self.month)
    }
}
...
impl Dangi {
    ...
    pub fn from_solar(solar: Solar) -> Result<Self, LunarError> {
        if solar.year() < 1 {
            return Err(LunarError::Parse(format!("unsupported dangi calendar date: {}", solar.to_ymd())));
        }
    ...
    pub fn dangi_month(&self) -> DangiMonth {
        DangiMonth::from_ym(self.year, self.month).unwrap()
    }
    ...
    pub fn to_string_in_lang(&self, language: crate::i18n::Language) -> String {
        crate::i18n::locale(language).render_dangi_string(self)
    }
    ...
    pub fn to_full_string_in_lang(&self, language: crate::i18n::Language) -> String {
        crate::i18n::locale(language).render_dangi_full(self)
    }
}

impl CalendarDay for Dangi {
    fn solar(&self) -> Solar {
        Dangi::solar(self)
    }
}

impl fmt::Display for Dangi {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "檀纪{}年{}月{}日", self.year, self.month, self.day)
    }
}
