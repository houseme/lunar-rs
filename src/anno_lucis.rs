...
/// 光明纪年 = 公历年份 + 4000。
pub const YEAR_OFFSET: i32 = 4_000;

#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash)]
pub struct AnnoLucisYear {
    year: i32,
}
...
impl AnnoLucisYear {
    pub fn from_year(year: i32) -> Result<Self, LunarError> {
        if year < YEAR_OFFSET + 1 {
            return Err(LunarError::Parse(format!("illegal anno lucis year: {year}")));
        }
        Ok(Self { year })
    }
    ...
    pub fn first_month(&self) -> AnnoLucisMonth {
        AnnoLucisMonth::from_ym(self.year, 1).unwrap()
    }

    pub fn last_month(&self) -> AnnoLucisMonth {
        AnnoLucisMonth::from_ym(self.year, 12).unwrap()
    }

    pub fn months(&self) -> Vec<AnnoLucisMonth> {
        (1..=12).map(|month| AnnoLucisMonth::from_ym(self.year, month).unwrap()).collect()
    }

    pub fn first_day(&self) -> AnnoLucis {
        self.first_month().first_day()
    }

    pub fn last_day(&self) -> AnnoLucis {
    ...
    pub fn contains_anno_lucis(&self, anno_lucis: AnnoLucis) -> bool {
        anno_lucis.year() == self.year
    }
...
impl CalendarSpan for AnnoLucisYear {
    fn first_solar_day(&self) -> Solar {
        AnnoLucisYear::first_solar_day(self)
    }

    fn last_solar_day(&self) -> Solar {
        AnnoLucisYear::last_solar_day(self)
    }
}

impl fmt::Display for AnnoLucisYear {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "光明纪年{}年", self.year)
    }
}

#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash)]
pub struct AnnoLucisMonth {
    year: i32,
    month: i32,
}
...
impl AnnoLucisMonth {
    ...
    pub fn anno_lucis_year(&self) -> AnnoLucisYear {
        AnnoLucisYear::from_year(self.year).unwrap()
    }
    ...
    pub fn first_day(&self) -> AnnoLucis {
        AnnoLucis::from_ymd(self.year, self.month, 1).unwrap()
    }

    pub fn last_day(&self) -> AnnoLucis {
        AnnoLucis::from_ymd(self.year, self.month, self.day_count()).unwrap()
    }
    ...
    pub fn days(&self) -> Vec<AnnoLucis> {
        (1..=self.day_count()).map(|day| AnnoLucis::from_ymd(self.year, self.month, day).unwrap()).collect()
    }
    ...
    pub fn contains_anno_lucis(&self, anno_lucis: AnnoLucis) -> bool {
        anno_lucis.year() == self.year && anno_lucis.month() == self.month
    }
...
impl CalendarSpan for AnnoLucisMonth {
    fn first_solar_day(&self) -> Solar {
        AnnoLucisMonth::first_solar_day(self)
    }

    fn last_solar_day(&self) -> Solar {
        AnnoLucisMonth::last_solar_day(self)
    }
}

impl fmt::Display for AnnoLucisMonth {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{}{}月", self.anno_lucis_year(), self.month)
    }
}

#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash)]
pub struct AnnoLucis {
    year: i32,
    month: i32,
    day: i32,
}
...
impl AnnoLucis {
    pub fn from_ymd(year: i32, month: i32, day: i32) -> Result<Self, LunarError> {
        AnnoLucisMonth::from_ym(year, month)?;
    ...
    pub fn from_solar(solar: Solar) -> Result<Self, LunarError> {
        if solar.year() < 1 {
            return Err(LunarError::Parse(format!("unsupported anno lucis date: {}", solar.to_ymd())));
        }
    ...
    pub fn anno_lucis_month(&self) -> AnnoLucisMonth {
        AnnoLucisMonth::from_ym(self.year, self.month).unwrap()
    }
    ...
    pub fn to_string_in_lang(&self, language: crate::i18n::Language) -> String {
        crate::i18n::locale(language).render_anno_lucis_string(self)
    }
    ...
    pub fn to_full_string_in_lang(&self, language: crate::i18n::Language) -> String {
        crate::i18n::locale(language).render_anno_lucis_full(self)
    }
}

impl CalendarDay for AnnoLucis {
    fn solar(&self) -> Solar {
        AnnoLucis::solar(self)
    }
}

impl fmt::Display for AnnoLucis {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "光明纪年{}年{}月{}日", self.year, self.month, self.day)
    }
}
