//! 全新世纪年（Holocene calendar / Human Era）。
...
impl HoloceneYear {
    pub fn from_year(year: i32) -> Result<Self, LunarError> {
        if year < YEAR_OFFSET + 1 {
            return Err(LunarError::Parse(format!("illegal holocene year: {year}")));
        }
        Ok(Self { year })
    }
    ...
    pub fn first_month(&self) -> HoloceneMonth {
        HoloceneMonth::from_ym(self.year, 1).unwrap()
    }

    pub fn last_month(&self) -> HoloceneMonth {
        HoloceneMonth::from_ym(self.year, 12).unwrap()
    }

    pub fn months(&self) -> Vec<HoloceneMonth> {
        (1..=12).map(|month| HoloceneMonth::from_ym(self.year, month).unwrap()).collect()
    }

    pub fn first_day(&self) -> Holocene {
        self.first_month().first_day()
    }

    pub fn last_day(&self) -> Holocene {
    ...
    pub fn contains_holocene(&self, holocene: Holocene) -> bool {
        holocene.year() == self.year
    }
...
impl CalendarSpan for HoloceneYear {
    fn first_solar_day(&self) -> Solar {
        HoloceneYear::first_solar_day(self)
    }

    fn last_solar_day(&self) -> Solar {
        HoloceneYear::last_solar_day(self)
    }
}

impl fmt::Display for HoloceneYear {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "全新世纪{}年", self.year)
    }
}

#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash)]
pub struct HoloceneMonth {
    year: i32,
    month: i32,
}
...
impl HoloceneMonth {
    ...
    pub fn holocene_year(&self) -> HoloceneYear {
        HoloceneYear::from_year(self.year).unwrap()
    }
    ...
    pub fn first_day(&self) -> Holocene {
        Holocene::from_ymd(self.year, self.month, 1).unwrap()
    }

    pub fn last_day(&self) -> Holocene {
        Holocene::from_ymd(self.year, self.month, self.day_count()).unwrap()
    }
    ...
    pub fn days(&self) -> Vec<Holocene> {
        (1..=self.day_count()).map(|day| Holocene::from_ymd(self.year, self.month, day).unwrap()).collect()
    }
    ...
    pub fn contains_holocene(&self, holocene: Holocene) -> bool {
        holocene.year() == self.year && holocene.month() == self.month
    }
...
impl CalendarSpan for HoloceneMonth {
    fn first_solar_day(&self) -> Solar {
        HoloceneMonth::first_solar_day(self)
    }

    fn last_solar_day(&self) -> Solar {
        HoloceneMonth::last_solar_day(self)
    }
}

impl fmt::Display for HoloceneMonth {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{}{}月", self.holocene_year(), self.month)
    }
}
...
impl Holocene {
    pub fn from_ymd(year: i32, month: i32, day: i32) -> Result<Self, LunarError> {
        HoloceneMonth::from_ym(year, month)?;
    ...
    pub fn from_solar(solar: Solar) -> Result<Self, LunarError> {
        if solar.year() < 1 {
            return Err(LunarError::Parse(format!("unsupported holocene calendar date: {}", solar.to_ymd())));
        }
    ...
    pub fn holocene_month(&self) -> HoloceneMonth {
        HoloceneMonth::from_ym(self.year, self.month).unwrap()
    }
    ...
    pub fn to_string_in_lang(&self, language: crate::i18n::Language) -> String {
        crate::i18n::locale(language).render_holocene_string(self)
    }
    ...
    pub fn to_full_string_in_lang(&self, language: crate::i18n::Language) -> String {
        crate::i18n::locale(language).render_holocene_full(self)
    }
}

impl CalendarDay for Holocene {
    fn solar(&self) -> Solar {
        Holocene::solar(self)
    }
}
...
impl fmt::Display for Holocene {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "全新世纪{}年{}月{}日", self.year, self.month, self.day)
    }
}
