...
/// - 1-3 月：公历年 - 1782
/// - 4-12 月：公历年 - 1781
pub const EARLY_YEAR_OFFSET: i32 = -1782;
pub const LATE_YEAR_OFFSET: i32 = -1781;
const SUPPORTED_START: (i32, i32, i32) = (1782, 4, 1);

fn rattanakosin_year_from_solar(solar: Solar) -> i32 {
...
pub struct RattanakosinYear {
    year: i32,
}

impl RattanakosinYear {
    pub fn from_year(year: i32) -> Result<Self, LunarError> {
        if year < 1 {
            return Err(LunarError::Parse(format!("illegal rattanakosin year: {year}")));
        }
        Ok(Self { year })
    }
    ...
    pub fn from_solar(solar: Solar) -> Result<Self, LunarError> {
        if (solar.year(), solar.month(), solar.day()) < SUPPORTED_START {
            return Err(LunarError::Parse(format!("unsupported rattanakosin date: {}", solar.to_ymd())));
        }
        Self::from_year(rattanakosin_year_from_solar(solar))
    }
    ...
    pub fn first_month(&self) -> RattanakosinMonth {
        RattanakosinMonth::from_ym(self.year, 4).unwrap()
    }

    pub fn last_month(&self) -> RattanakosinMonth {
        RattanakosinMonth::from_ym(self.year, 3).unwrap()
    }

    pub fn months(&self) -> Vec<RattanakosinMonth> {
        let mut months = Vec::with_capacity(12);
        for month in 4..=12 {
            months.push(RattanakosinMonth::from_ym(self.year, month).unwrap());
        }
        for month in 1..=3 {
            months.push(RattanakosinMonth::from_ym(self.year, month).unwrap());
        }
    ...
    pub fn day_count(&self) -> i32 {
        self.months().iter().map(RattanakosinMonth::day_count).sum()
    }

    pub fn first_day(&self) -> Rattanakosin {
        Rattanakosin::from_ymd(self.year, 4, 1).unwrap()
    }

    pub fn last_day(&self) -> Rattanakosin {
        Rattanakosin::from_ymd(self.year, 3, 31).unwrap()
    }
    ...
    pub fn contains_rattanakosin(&self, date: Rattanakosin) -> bool {
        date.year() == self.year
    }
...
impl CalendarSpan for RattanakosinYear {
    fn first_solar_day(&self) -> Solar {
        RattanakosinYear::first_solar_day(self)
    }

    fn last_solar_day(&self) -> Solar {
        RattanakosinYear::last_solar_day(self)
    }
}

impl fmt::Display for RattanakosinYear {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "拉达那哥欣{}年", self.year)
    }
}

#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash)]
pub struct RattanakosinMonth {
    year: i32,
    month: i32,
}

impl RattanakosinMonth {
    pub fn from_ym(year: i32, month: i32) -> Result<Self, LunarError> {
        RattanakosinYear::from_year(year)?;
        if !(1..=12).contains(&month) {
            return Err(LunarError::Parse(format!("illegal rattanakosin month: {year}-{month}")));
        }
        Ok(Self { year, month })
    }

    pub fn from_solar(solar: Solar) -> Result<Self, LunarError> {
        let date = Rattanakosin::from_solar(solar)?;
    ...
    pub fn rattanakosin_year(&self) -> RattanakosinYear {
        RattanakosinYear::from_year(self.year).unwrap()
    }
    ...
    pub fn first_day(&self) -> Rattanakosin {
        Rattanakosin::from_ymd(self.year, self.month, 1).unwrap()
    }

    pub fn last_day(&self) -> Rattanakosin {
        Rattanakosin::from_ymd(self.year, self.month, self.day_count()).unwrap()
    }
    ...
    pub fn days(&self) -> Vec<Rattanakosin> {
        (1..=self.day_count()).map(|day| Rattanakosin::from_ymd(self.year, self.month, day).unwrap()).collect()
    }
...
impl CalendarSpan for RattanakosinMonth {
    fn first_solar_day(&self) -> Solar {
        RattanakosinMonth::first_solar_day(self)
    }

    fn last_solar_day(&self) -> Solar {
        RattanakosinMonth::last_solar_day(self)
    }
}

impl fmt::Display for RattanakosinMonth {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{}{}月", self.rattanakosin_year(), self.month)
    }
}

#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash)]
pub struct Rattanakosin {
    year: i32,
    month: i32,
    day: i32,
}
...
impl Rattanakosin {
    pub fn from_ymd(year: i32, month: i32, day: i32) -> Result<Self, LunarError> {
        let month_obj = RattanakosinMonth::from_ym(year, month)?;
    ...
    pub fn from_solar(solar: Solar) -> Result<Self, LunarError> {
        ...
        Ok(Self { year: rattanakosin_year_from_solar(solar), month: solar.month(), day: solar.day() })
    }
    ...
    pub fn rattanakosin_month(&self) -> RattanakosinMonth {
        RattanakosinMonth::from_ym(self.year, self.month).unwrap()
    }
    ...
    pub fn to_string_in_lang(&self, language: crate::i18n::Language) -> String {
        crate::i18n::locale(language).render_rattanakosin_string(self)
    }
    ...
    pub fn to_full_string_in_lang(&self, language: crate::i18n::Language) -> String {
        crate::i18n::locale(language).render_rattanakosin_full(self)
    }
}

impl CalendarDay for Rattanakosin {
    fn solar(&self) -> Solar {
        Rattanakosin::solar(self)
    }
}

impl fmt::Display for Rattanakosin {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "拉达那哥欣{}年{}月{}日", self.year, self.month, self.day)
    }
}
