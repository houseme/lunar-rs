...
impl<'a> DaYun<'a> {
    ...
    pub const fn end_year(&self) -> i32 { self.end_year }
    pub const fn start_age(&self) -> i32 { self.start_age }
    pub const fn end_age(&self) -> i32 { self.end_age }
    ...
    pub fn gan_zhi(&self) -> String {
        ...
        if self.forward { offset += self.index as i64; } else { offset -= self.index as i64; }
    ...
    pub fn liu_nian(&self) -> Vec<LiuNian<'a>> { self.liu_nian_by(10) }
    pub fn liu_nian_by(&self, mut n: i32) -> Vec<LiuNian<'a>> {
        if self.index < 1 {
            n = self.end_year - self.start_year + 1;
        }
        (0..n)
            .map(|i| LiuNian::new(self.lunar, self.start_year, self.start_age, self.index, i))
            .collect()
    }
    pub fn xiao_yun(&self) -> Vec<XiaoYun<'a>> { self.xiao_yun_by(10) }
    pub fn xiao_yun_by(&self, mut n: i32) -> Vec<XiaoYun<'a>> {
        if self.index < 1 {
            n = self.end_year - self.start_year + 1;
        }
        (0..n)
            .map(|i| XiaoYun::new(self.lunar, self.start_year, self.start_age, self.index, i, self.forward))
            .collect()
    }
}
