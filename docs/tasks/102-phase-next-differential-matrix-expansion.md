...
前面 `Task 97` - `Task 100` 已补完 `SolarFestival` / `LunarFestival`、`LunarYear` / `LunarMonth` 严格 getter，以及本地 `Foto` / `Tao` wrapper 兼容层，但外部差分校验还只覆盖：
...
这意味着最近新增的兼容面即使回归，现有差分协议也感知不到。因此本任务把差分协议升级到 v2，并把新增协议字段落到 sample matrix 和本地稳定性测试里。
