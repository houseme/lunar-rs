...
| `P2` | `Task 102` | 差分协议与样例矩阵扩容 | `已完成` | 将差分协议扩展到当前 `v8`，并把节日 wrapper、闰月、星期、星座、法定假日、文化日对象、`LunarHour` 与 `LunarYear/LunarMonth` 稳定字段纳入 sample matrix。 |
...
| `P2` | `Task 105` | tyme4rs 外部 reference bridge 落地 | `已完成` | 新增可执行脚本并实际跑通 `tyme4rs` `v8` 差分；同时补齐月份中文同义归一化与节气节日年份基准。 |
