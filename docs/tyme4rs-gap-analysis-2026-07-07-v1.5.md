...
| `P2` | `Task 102` | 差分协议与样例矩阵扩容 | `已完成` | 将差分协议扩展到当前 `v6`，并把节日 wrapper、闰月、星期、星座、法定假日、文化日对象与 `LunarHour` 稳定字段纳入 sample matrix。 |
...
| `P2` | `Task 105` | tyme4rs 外部 reference bridge 落地 | `已完成` | 新增可执行脚本并实际跑通 `tyme4rs` `v6` 差分；同时补齐月份中文同义归一化与节气节日年份基准。 |
...
| `P2` | `Task 108` | 差分中的 LunarYear/LunarMonth 文化字段扩容 | `已完成` | 将差分协议扩展到 `v7`，新增 `LunarYear/LunarMonth` 稳定文化字段。 |
