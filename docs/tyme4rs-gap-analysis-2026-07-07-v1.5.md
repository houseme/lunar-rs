...
| `P2` | `Task 105` | tyme4rs 外部 reference bridge 落地 | `已完成` | 新增可执行脚本并实际跑通 `tyme4rs` v2 差分；同时补齐月份中文同义归一化与节气节日年份基准。 |
